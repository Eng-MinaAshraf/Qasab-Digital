    ...
    try {
      ...
      const intentionRes = await fetch('https://accept.paymob.com/v1/intention/', {
        ...
        body: JSON.stringify({
          ...
          items: (items || []).map((i) => ({
            name: i.title,
            amount: Math.round((i.unitPrice || 20) * 100),
            quantity: i.quantity || 1,
            description: `عصير قصب - ${i.size || 'وسط'}`,
          })),
          billing_data: {
            first_name: customer.name?.split(' ')[0] || 'عميل',
            last_name: customer.name?.split(' ').slice(1).join(' ') || 'قصب',
  ...
  try {
    ...
    const orderRes = await fetch('https://accept.paymob.com/api/ecommerce/orders', {
      ...
      body: JSON.stringify({
        ...
        items: (items || []).map((i) => ({
          name: i.title,
          amount_cents: Math.round((i.unitPrice || 20) * 100),
          quantity: i.quantity || 1,
        })),
      }),
    });
